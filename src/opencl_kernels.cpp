...
/*
 * Create and initialize all the platform, context, queue and devices
 */
...
/*
 * Pass name of kernel as string to get back size of program and loaded kernel
 */

char * load_kernel(const std::string s, size_t *programSize) {
    FILE* programHandle;
    char *programBuffer;
    char program[s.length()+1];
    // Variables
    strcpy(program, s.c_str());
    programHandle = fopen(program, "r");
...
/*
 * Set up all relevant data for call to OpenCL kernel
 */
...
cv::Mat openCLKernel::gaussian_blur(const cv::Mat & frame, int kernelSize, float sigma) {
    ...
    size_t globalSize, localSize; char *programBuffer;
    size_t programSize;
